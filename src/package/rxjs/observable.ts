...
error_observable.subscribe({
  ...
  error: function (err) {
    // 捕获未定义的a变量错误, 不影响 上一个observable 的 定时器执行
...
console.log(subscription_2);

// -------------- operator ------------
const from_observable = from([10, 20, 30]);
from_observable.subscribe((x) => {
  console.log('form:', x); // 10, 20, 30
});

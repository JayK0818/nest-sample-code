...
(Nest 在方法调用之前插入 管道，并且接受函数的 参数进行操作。比如数据的验证或者转换)
...
PipeTransform 是一个泛型接口。 T 表示输入的 value 类型, R 表示经过 transform() 方法转换后的类型。每个管道都必须实现 transform() 方法。
